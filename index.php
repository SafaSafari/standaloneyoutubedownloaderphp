<?php
if (!isset($_GET['v'])) exit('v');
$yt = new YouTubeDownloader();
if (isset($_GET['d'])) exit(nl2br(print_r($yt->getDownloadLinks($_GET['v'], false), true)));
$links = $yt->getDownloadLinks($_GET['v']);
if (isset($_GET['a'])) exit(implode(PHP_EOL, $links));
echo nl2br('video_360p: ' . $links[0] . PHP_EOL . PHP_EOL . PHP_EOL . PHP_EOL . 'video: ' . $links[1] . PHP_EOL . PHP_EOL . PHP_EOL . PHP_EOL . 'audio: ' . $links[2] . PHP_EOL . PHP_EOL . PHP_EOL . $yt->getLastError());
class Browser
{
    ...
    public function get($url)
    {
        ...
        if ($this->proxy) {
            $part = explode(':', $this->proxy);
            curl_setopt($ch, CURLOPT_PROXYTYPE, CURLPROXY_HTTPS);
            curl_setopt($ch, CURLOPT_PROXY, $part[0] . ':' . $part[1]);
            curl_setopt($ch, CURLOPT_PROXYUSERPWD, $part[2] . ':' . $part[3]);
        }
    ...
    public function parseFunctionCode($func_name, $player_html)
    {
        ...
        if (preg_match('/' . $func_name . '=function\([a-z]+\){(.*?)}/', $player_html, $matches)) {
            ...
            if (preg_match_all('/([a-z0-9$]{2})\.([a-z0-9]{2})\([^,]+,(\d+)\)/i', $js_code, $matches) != false) {
                ...
                preg_match_all('/(' . implode('|', (array) $func_list) . '):function(.*?)\}/m', $player_html, $matches2, PREG_SET_ORDER);
                ...
                foreach ((array) $matches[2] as $index => $name) {
...
class YouTubeDownloader
{
    ...
    public function getDownloadLinks($video_id, $selector = [22, 140, 18])
    {
        $this->error = null;
